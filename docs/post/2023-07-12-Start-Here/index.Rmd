...
a <- as.data.frame(a)
colnames(a) <- " "
a$Program <- c("Excel","Prism","SPSS","R")

tile <- a[1:4, 1:2]
colnames(tile) <- c("ID", "Program")
tile$`Learning curve` <- c(1,2,2,4)
tile$Code <- c(0,0,5,5)
...
empty <- tile
empty$`Learning curve` <- c(0,0,0,0)
empty$Code <- c(0,0,0,0)
empty$Graphs <- c(0,0,0,0)
empty$Analyses <- c(0,0,0,0)
empty$Cost <- c(0,0,0,0)
empty$`Data storage` <- c(0,0,0,0)
empty$Community <- c(0,0,0,0)
...
```{r, include=FALSE}
m_tile <- melt(tile, id.vars=c("ID", "Program"))
m_empty <- melt(empty, id.vars=c("ID", "Program"))

a <- ggplot(m_empty, aes(x=variable,y=Program,fill=value))+
  geom_tile()+
  coord_fixed()+ 
...
ggsave("empty.png",a,height=5,width=10,dpi=300)
ggsave("tilez.png",b,height=5,width=10,dpi=300)
```
...
```{r, echo=FALSE}
knitr::include_graphics("tilez.png")
```
